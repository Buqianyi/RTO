...
while cap.isOpened():
    ...
    info_texts = [
        ...
        f'Novel Vectors 5: {novel_vectors[0]}',
        f'Min Distance 5: {min_distance[0]:.2f}',
        f'Novel Vectors 10: {novel_vectors[1]}',
        f'Min Distance 10: {min_distance[1]:.2f}',
        f'Novel Vectors 15: {novel_vectors[2]}',
        f'Min Distance 15: {min_distance[2]:.2f}',
        f'Min Dist (Pre-Reduction): {min_dist_before_dim_reduction:.2f}',
...
colors = ['b', 'g', 'r']
for i, pca in enumerate(pca_models):
    plt.plot(pca['timestamps'], pca['novel_vectors'], label=f'PCA Model {pca["d"]} (Threshold={pca["threshold"]})', color=colors[i])
plt.xlabel('Time (seconds)')
plt.ylabel('Novel Vectors')
plt.title('Novel Vectors Over Time for Different PCA Models')
plt.legend()
plt.grid(True)
plt.show()
